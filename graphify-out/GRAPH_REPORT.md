...
## Corpus Check
- 92 files · ~47,638 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 766 nodes · 1213 edges · 60 communities (55 shown, 5 thin omitted)
...
2. `User` - 32 edges
3. `seed_database()` - 21 edges
4. `Alert` - 19 edges
5. `AlertService` - 18 edges
6. `compilerOptions` - 17 edges
7. `Tariff Resilience` - 15 edges
8. `PaginatedResponse` - 14 edges
9. `ComplianceWorkflowService` - 14 edges
10. `MonitoringService` - 14 edges
...
### Community 2 - "main.py"
Cohesion: 0.06
Nodes (33): Application configuration using pydantic-settings., Parse CORS origins from comma-separated string., Application settings loaded from environment variables., Settings, Config, create_access_token(), login(), LoginRequest (+25 more)
...
Cohesion: 0.15
Nodes (21): Database models package., Country, DistributionCenter, Port, Product, Reference data models: suppliers, products, countries, lanes, routes, ports, DCs, Supplier reference data., Product reference data. (+13 more)
...
Cohesion: 0.22
Nodes (7): Audit entry model for immutable history., Base declarative model and common columns., Mixin for created_at and updated_at timestamps., TimestampMixin, Compliance review and risk flag models., Risk flag for compliance concerns., RiskFlag
...
Cohesion: 0.18
Nodes (13): Alert, Material impact alert., Tests for alerts router., Test alert resolution., Test list alerts with filters., Test list alerts when none exist., Test alert acknowledgement., Test alert escalation. (+5 more)
...
Cohesion: 0.06
Nodes (27): Role, create_test_user(), hash_password(), override_get_db(), Unit tests for authentication endpoints., Test login with non-existent email., Test login with incorrect password., Test login with inactive user account. (+19 more)

### Community 18 - "TradeAgreementService"
Cohesion: 0.24
Nodes (7): AgreementEvaluation, EvidenceGap, Trade agreement evaluation models., Evidence gaps preventing agreement qualification., Trade agreement qualification and savings evaluation., Trade agreement service for evaluation and qualification., Evaluate all applicable trade agreements for a scenario result.         Implemen
...
Cohesion: 0.13
Nodes (16): disposition_recommendation(), generate_recommendations(), get_recommendation(), get_recommendation_service(), Session, Recommendations router., Dependency injection for RecommendationService., Generate mitigation recommendations for a scenario.     Implements REQ-API-005: (+8 more)

### Community 31 - "agreements.py"
Cohesion: 0.14
Nodes (13): AcknowledgeAlertRequest, acknowledge_alert(), escalate_alert(), get_alert(), list_alerts(), Escalate alert to higher scope.     Implements REQ-API-006: Alert escalation end, List alerts with filters and pagination.     Implements REQ-API-006: Alert list, Get alert detail with transitions.     Implements REQ-API-006: Alert detail endp (+5 more)

### Community 32 - "recommendation.py"
Cohesion: 0.16
Nodes (14): User and authentication models., User model with Integer PK., User, get_monitoring_service(), get_policy_event(), ingest_policy_event(), list_policy_events(), Session (+6 more)
...
Cohesion: 0.24
Nodes (9): FastAPI dependencies for dependency injection., client(), create_test_app(), db_session(), Test fixtures and configuration., Create a test FastAPI app without lifespan., Create a fresh database session for each test., Create a test client with overridden database dependency. (+1 more)
...
Cohesion: 0.14
Nodes (14): get_agreement_evaluation(), get_agreement_service(), get_scenario_agreement_evaluations(), Session, Trade agreements router., Dependency injection for TradeAgreementService., Get agreement evaluations for a scenario result.     Implements REQ-API-004: Tra, Get single agreement evaluation with evidence gaps.     Implements REQ-API-004: (+6 more)
...
Cohesion: 0.20
Nodes (6): AlertTransition, Alert and notification models., Alert state transition history., Alert service for alert orchestration and routing., Escalate alert to higher scope.         Implements FR-AL-04: Escalation paths., Resolve alert with outcome summary.         Implements FR-AL-07, BR-AL-02: Alert
...
### Community 50 - "verify_auth.py"
Cohesion: 0.22
Nodes (5): Alert, List alerts with filters and pagination., Create alert with routing based on severity and ownership.         Implements FR, Acknowledge alert.         Implements FR-AL-03: Acknowledgement tracking., Assign or reassign alert to an owner.

### Community 51 - ".create_scenario_request"
Cohesion: 0.10
Nodes (15): init_db(), Initialize database tables., health_check(), lifespan(), Request, Main FastAPI application entrypoint., Add security headers to all responses (REQ-SEC-001)., Application lifespan manager for startup and shutdown events. (+7 more)

### Community 53 - "Recommendation"
Cohesion: 0.24
Nodes (7): Mitigation recommendation models., Factor contributing to recommendation score and rationale., Mitigation recommendation., Recommendation, RecommendationFactor, Recommendation service for mitigation strategy generation., Generate mitigation recommendations for a scenario.         Implements FR-MR-01

### Community 54 - "alerts.py"
Cohesion: 0.33
Nodes (7): get_current_user(), get_current_user_optional(), Session, Authentication middleware for JWT token validation., Validate JWT token and return current user.          REQ-AUTH-001: JWT token val, Optional authentication dependency.     Returns user if valid token provided, No, HTTPAuthorizationCredentials

### Community 55 - "database.py"
Cohesion: 0.40
Nodes (4): get_db(), Session, Database connection and session management., Dependency to get database session.

### Community 56 - "assign_alert"
Cohesion: 0.67
Nodes (3): AssignAlertRequest, assign_alert(), Assign or reassign alert to owner.     Implements REQ-API-006: Alert assignment
...
### Community 58 - "get_alert_service"
Cohesion: 0.67
Nodes (3): get_alert_service(), Session, Dependency injection for AlertService.

### Community 59 - "resolve_alert"
Cohesion: 0.67
Nodes (3): Resolve alert with outcome summary.     Implements REQ-API-006: Alert resolution, resolve_alert(), ResolveAlertRequest
...
- **Why does `BaseModel` connect `Recommendation` to `main.py`, `package.json`, `config.py`, `dependencies`, `Local Development Setup`, `alerts.py`, `MonitoringService`, `recommendations.py`, `TradeAgreementService`, `AuditEntry`, `scenario.py`, `recommendation.py`, `scenario.py`, `scenario.py`, `dependencies.py`, `config.py`, `seed_database`, `compliance.py`, `auth.py`, `AgreementEvaluationResponse`, `common.py`, `Recommendation`?**
  _High betweenness centrality (0.140) - this node is a cross-community bridge._
- **Why does `User` connect `recommendation.py` to `main.py`, `package.json`, `dependencies.py`, `Troubleshooting`, `recommendations.py`, `alerts.py`, `Recommendation`, `test_scenarios.py`?**
  _High betweenness centrality (0.078) - this node is a cross-community bridge._
- **Why does `Alert` connect `alerts.py` to `main.py`, `package.json`, `seed_database`, `verify_auth.py`, `alerts.py`, `Recommendation`, `agreements.py`?**
...
- **Are the 11 inferred relationships involving `User` (e.g. with `BaseModel` and `Config`) actually correct?**
