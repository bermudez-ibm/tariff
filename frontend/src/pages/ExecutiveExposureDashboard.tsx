...
export default function ExecutiveExposureDashboard() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        setLoading(true);
        const [exposureRes, alertsRes] = await Promise.all([
          request<{ items: any[]; total: number } | any[]>('/dashboard/exposure'),
          request<{ items: Alert[]; total: number } | Alert[]>('/alerts')
        ]);
        
        // Defensive pagination wrapper extraction
        const exposureItems = Array.isArray(exposureRes) ? exposureRes : exposureRes?.items || [];
        const alertItems = Array.isArray(alertsRes) ? alertsRes : alertsRes?.items || [];
              ...
              <tbody>
                {alerts.map((alert) => (
                  <tr key={alert.id} className="hover:bg-slate-50">
                    ...
                    <td className="py-4 px-4 text-sm border-b border-slate-100">
                      <a href={`/alerts/${alert.id}`} className="text-blue-700 font-medium hover:underline">View Details →</a>
                    </td>
                  </tr>
                ))}
              </tbody>
